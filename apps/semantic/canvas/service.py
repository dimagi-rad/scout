    ...
    if object_type == "field":
        ...
        if key not in allowed:
            raise CanvasOperationError(
                ...
                + (
                    " (autogenerated fields accept label/description curation only)."
...
def _op_delete_object(canvas, model, index, raw_op, user) -> dict[str, Any]:
    ...
    if object_type == "field":
        field = _resolve(index, resolve_field, model, ref)
        if not is_canvas_created(field):
            raise CanvasOperationError(
                index,
                "PROTECTED_FIELD",
                "Autogenerated fields cannot be removed. Only fields created through "
                "the canvas can be deleted.",
            )
