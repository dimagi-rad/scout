...
class SemanticField(models.Model):
    ...
    class MeasureType(models.TextChoices):
        ...
        COUNT_DISTINCT_APPROX = "count_distinct_approx", "Approximate count distinct"
        SUM = "sum", "Sum"
        AVG = "avg", "Average"
        MIN = "min", "Minimum"
        MAX = "max", "Maximum"
    ...
    measure_type = models.CharField(max_length=32, choices=MeasureType.choices, blank=True)
    is_visible = models.BooleanField(default=True)
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
