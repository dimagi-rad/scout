...
#:
#: These are predicates rather than a function returning the code, so that nothing
#: derived from the response body is ever passed to a logger. CodeQL's taint analysis
#: follows data, not control flow: a call reading response.json() is safe in a
#: condition, but flagged as clear-text logging the moment its result is an argument.
