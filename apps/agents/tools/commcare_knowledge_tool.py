...
def create_commcare_knowledge_tool(
    ...
    async def download_commcare_knowledge() -> dict[str, Any]:
        ...
        for app_key in ("learn_app", "deliver_app"):
            ...
            app_def = await sync_to_async(fetch_app_by_id)(domain, app_id, session=session)
        ...
        if entries_created:
            summary_parts.append(
                f"Created {len(entries_created)} knowledge entries: {', '.join(entries_created)}"
            )
        if entries_updated:
            summary_parts.append(
                f"Updated {len(entries_updated)} knowledge entries: {', '.join(entries_updated)}"
            )
        if apps_failed:
            summary_parts.append(
                f"Warning: could not fetch {len(apps_failed)} apps: {', '.join(apps_failed)}"
            )

        total_forms = sum(len(extract_form_definitions([app])) for app in apps_fetched)
