...
class TestToolStream:
    """Tests for tool call streaming in langgraph_to_ui_stream."""

    @pytest.mark.asyncio
    async def test_run_materialization_tool_produces_card(self):
        """run_materialization should produce tool-input-available and tool-output-available via on_tool_end."""
        mock_agent = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_all_tools_use_tool_end_path(self):
        """All tools use the on_tool_end path for card open."""
        mock_agent = AsyncMock()
