...
def provisioning_schema(db, tenant, workspace, tenant_membership_obj):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "INSERT INTO procrastinate_jobs (id, queue_name, task_name, status, args) "
            "VALUES (%s, %s, %s, %s::procrastinate_job_status, %s::jsonb)",
            [
                438,
                "default",
    ...
    schema.refresh_job_id = 438
    schema.save(update_fields=["refresh_job_id"])
    yield schema
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM procrastinate_jobs WHERE id = %s", [438])
