...
Fetches payment records from the Connect CSV export endpoint.
"""
...
class ConnectPaymentLoader(ConnectBaseLoader):
    ...
    def load_pages(self) -> Iterator[list[dict]]:
        url = self._opp_url("payment/")
        rows = self._get_csv(url)
        logger.info("Fetched %d payments for opportunity %s", len(rows), self.opportunity_id)
        if rows:
            yield rows
