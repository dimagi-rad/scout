...
Fetches completed module records from the Connect CSV export endpoint.
"""
...
class ConnectCompletedModuleLoader(ConnectBaseLoader):
    ...
    def load_pages(self) -> Iterator[list[dict]]:
        url = self._opp_url("completed_module/")
        rows = self._get_csv(url)
        logger.info(
            "Fetched %d completed modules for opportunity %s", len(rows), self.opportunity_id
        )
        if rows:
            yield rows
